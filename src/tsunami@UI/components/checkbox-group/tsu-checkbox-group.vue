<template>
    <div class="tsu-checkbox-group" 
        :class="[(direction === 'col')
        // TODO: поменять стили на локальные для компонента,
        // либо вынести в локальный пак для UI 
...
const modelValue = ref<T[]>([]) as Ref<T[]>;

const props = withDefaults(
    defineProps<ICheckboxGroupProps<T>>(), {
        type: 'checkbox',
        mode: 'multiplay',
        direction: 'row',
        options: () => []
...
// TODO: как сделать грамотную передачу элемента,
// если он единственный, а не массив ? 
